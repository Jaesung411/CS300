...
public class Test2 {
		...
		public static boolean testSendClimb() {
			...
			ClimbingTracker1.sendClimb(sendArray1, num1[0], grade);
			if(!(ClimbingTracker1.sendClimb(sendArray1, num1[0], grade) == expected1)) {
			...
			for(int i = 0; i < grade2.length; i++) {
				ClimbingTracker1.sendClimb(sendArray2, num2[i], grade2[i]);
				size2++;
			}
			printOversizeArray(sendArray2,size2);

			if(!(ClimbingTracker1.sendClimb(sendArray2, zero, grade) == expected2)) {
			...
			for(int i = 0; i < grade3.length; i++) {
				ClimbingTracker1.sendClimb(sendArray3, num3[i], grade3[i]);
				size3++;
			}
			if(!(ClimbingTracker1.sendClimb(sendArray3, zero, grade) == expected3)) {
		...
		public static boolean testFailClimb() {
			...
			ClimbingTracker1.failClimb(failArray1, num1[0], grade);
			if(!(ClimbingTracker1.failClimb(failArray1, num1[0], grade) == expected1)) {
				System.out.println("faliled test1");
				return false;
			}
			...
			for(int i = 0; i < num2.length-1; i++) {
				ClimbingTracker1.failClimb(failArray2, num2[i], grade2[i]);
				size2++;
			}
			printOversizeArray(failArray2, size2);
			if(!(ClimbingTracker1.failClimb(failArray2, zero, grade) == expected2)) {
				System.out.println("faliled test2");
				return false;
			}
			...
			for(int i = 0; i < grade3.length; i++) {
				ClimbingTracker1.failClimb(failArray3, num3[i], grade3[i]);
				size3++;
			}
			if(!(ClimbingTracker1.sendClimb(failArray3, zero, grade) == expected3)) {
				System.out.println("faliled test3");
				return false;
			}
		...
		public static boolean testGetStats(){
			...
			for(int i = 0; i < sendGrade1.length; i++) {
				ClimbingTracker2.sendClimb(sendArray1, sendNum1[i], sendGrade1[i]);
				System.out.println(sendArray1[i]);
				sendSize1++;
			}
			for(int i = 0; i < failGrade1.length; i++) {
				ClimbingTracker2.failClimb(failArray1, failNum1[i], failGrade1[i]);
				failSize1 += failNum1[i];
			}
			
			if(!(ClimbingTracker2.getStats(sendArray1, sendSize1, failArray1, failSize1, 2)==expected1)) {
				return false;
			}
