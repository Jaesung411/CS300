...
public class test {
  ...
  public static int failClimb(String[] fail, int numFail, String grade) 
    ...
    // check size
    if (numFail == fail.length) //제공되는 배열 꽉 차있는지 확
  ...
  public static String getStats(String[] send, int numSend, String[] fail, int numFail, 
      int historyLength) throws RuntimeException {
	  ...
	  //historyLength가 0 보다 작을때
	  if(historyLength <= 0)
		  throw new IllegalArgumentException(historyLength + " is not a valid history");

	  //historyLength가 0 보다 작을때
	  if(historyLength <= 0) throw new IllegalArgumentException(historyLength + " is not a valid history length");
